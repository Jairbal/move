...
const { agentFixtures } = require('move-utils')
...
const usernameArgs = {
  where: { username: 'move', connected: true }
}
...
test.beforeEach(async () => {
  ...
  AgentStub.findAll.withArgs(usernameArgs).returns(Promise.resolve(agentFixtures.move))
...
test.serial('Agent#findByUsername', async t => {
  const agents = await db.Agent.findByUsername('move')

  t.true(AgentStub.findAll.called, 'findAll should be called on model')
  t.true(AgentStub.findAll.calledOnce, ' findAll should be called once')
  t.true(AgentStub.findAll.calledWith(usernameArgs), 'findAll should be called with username args')

  t.is(agents.length, agentFixtures.move.length, 'agents should be the same amount')
  t.deepEqual(agents, agentFixtures.move, 'agents should be the same')
})
